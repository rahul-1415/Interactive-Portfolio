...
## 2026-07-05 — v2.4 decisions

- **Reward-gated invisibility reads as a bug.** Skypiea is always rendered; the
  _golden bell_ is the completion reward. Hide state, not content.
- **Music is an original composition, synthesized** — no One Piece OST (copyright),
  no audio files. An original 6/8 shanty keeps the spirit legally and at 0 bytes.
- **Audio defaults ON** because it starts on the Set Sail click (a user gesture, so
  autoplay-safe); a missing `music` key in old saves triggers a schema upgrade that
  re-defaults sound to on (the old default was off).
- **Poster art must be license-clean**: Openverse queries restricted to CC0/PDM only —
